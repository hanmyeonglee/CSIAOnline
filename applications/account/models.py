...
class User(models.Model):
    ...
    def __str__(self):
        return f"#{self.grade}-{self.classroom}-{str(self.number).ljust('0')}-{bin(self.auth)[2:]}"
...
class LoginSession(models.Model):
    ...
    def __str__(self):
        return f"{self.student} : session id={self.session}"
    ...
    def is_over(self):
        ...
        """
        return True if (datetime.now() - self.allot_time).seconds > 604800 else False
